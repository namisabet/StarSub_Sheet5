...
using StarSubsSheet5.Models;

namespace StarSubsSheet5.Controllers
{
    public class HomeController : Controller
    {
...

        {
            ...
            double totalProfit = preTaxPrice * Int32.Parse(subNumber);
            ...
            //Taxes
            if (Session["taxes"] == null)
            {
                double taxes = taxPrice;
                Session["taxes"] = taxes;
            }
            else
            {
                double sessionTax = (double)Session["taxes"];
                double totalTaxes = sessionTax + taxPrice;
                Session["taxes"] = totalTaxes;
            }

            //TotalIncome
            if (Session["profit"] == null)
            {
            ...
            {
                double sessionProfit = (double)Session["profit"];
                double totalProfit2 = sessionProfit + totalProfit;
                Session["profit"] = totalProfit2;
            }
